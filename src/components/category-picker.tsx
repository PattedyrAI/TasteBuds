...
export function CategoryPicker({value,onChange,categories,disabled=false,onCreate,canCreate=false}:{value:string;onChange:(value:string)=>void;categories:string[];disabled?:boolean;onCreate?:()=>void;canCreate?:boolean}){
 ...
 <SelectMenu label="Category" value={current} onChange={next=>{if(next==='__new_category__')onCreate?.();else onChange(next);}} disabled={disabled} searchable options={[...(canCreate&&onCreate?[{value:'__new_category__',label:'+ Lag kategori',alwaysVisible:true}]:[]),{value:'',label:'Choose a category'},...options.map(name=>({value:name,label:name}))]}/>
 </div>;
}
