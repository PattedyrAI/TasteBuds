...
export function Modal({title,close,children,className=''}:{title:string;close:()=>void;children:React.ReactNode;className?:string}){
  const titleId=useId();const ref=useRef<HTMLDialogElement>(null);useEffect(()=>{ref.current?.showModal();},[]);
  return <dialog ref={ref} className={`modal ${className}`} aria-labelledby={titleId} onCancel={event=>{event.preventDefault();event.stopPropagation();close();}}><div className="modal-head"><h2 id={titleId}>{title}</h2><button type="button" className="icon-button" aria-label="Close" onClick={close}><X size={22}/></button></div>{children}</dialog>;
}
