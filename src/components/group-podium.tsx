...
export function GroupPodium({items,open,browse}:{items:Item[];open:(id:string)=>void;browse:()=>void}){
 ...
 </button>)}</div>{ranked.length>3&&<div className="runners-up" aria-label="More group favourites">{ranked.slice(3).map((item,index)=><button className="ranking" key={item.id} onClick={()=>open(item.id)}><span className="rank">{index+4}</span><Photo id={item.photoId} name={item.name}/><div><strong>{item.name}</strong><BrandLabel brand={item.brand}/>{!item.brand&&<small>{item.type}</small>}</div><TastingScore item={item}/><ArrowUpRight size={16}/></button>)}</div>}</>:<div className="podium-empty"><Trophy size={36}/><h3>The podium is waiting.</h3><p>{category==='all'?'An item needs ratings from at least three people to earn a place.':'No items in this category have ratings from three people yet.'}</p><button className="button secondary" onClick={browse}>Explore the collection</button></div>}
 </section>;
}
