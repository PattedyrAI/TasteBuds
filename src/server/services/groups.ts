...
async function detail(db: Db, userId: string, groupId: string): Promise<GroupDetail> {
  ...
  const connections=canManageGroup(membership.role)?await db.query('SELECT route,enabled,category_ids FROM everrate.discord_connections WHERE group_id=$1 ORDER BY route',[groupId]):{rows:[]};
  const row = result.rows[0]; const s = stats.rows[0];
  return { discordConnections:connections.rows.map(c=>({route:c.route,enabled:c.enabled,categoryIds:c.category_ids})),categories:categories.rows.map(c=>({id:c.id,name:c.name,fields:c.fields})),...group({...row,role:membership.role}),inviteCode: canManageGroup(membership.role) ? row.invite_code : null,discordConnected: row.discord_connected,members: members.rows.map(row=>({...user(row),role:isPlatformAdmin(row.id)?'admin':row.role,joinedAt:iso(row.joined_at)})),stats:{ itemCount:Number(s.item_count),tastingCount:Number(s.tasting_count),activeMembers:Number(s.active_members) } };
}
