...
export interface Item { myScore?: number | null; saved?: boolean; reviewers?: Pick<User, 'id' | 'displayName' | 'avatarUrl'>[]; id: string; groupId: string; createdBy: string; name: string; brand: string | null; variant: string | null; type: string | null; broadCategory: string | null; photoId: string | null; average: number | null; raterCount: number; tastingCount: number; lastRatedAt: string | null }
export interface Comment { id: string; ratingId: string; author: User; body: string; createdAt: string }
export interface Rating { photoIds?: string[]; isRereview?: boolean; countsTowardAverage?: boolean; id: string; groupId: string; itemId: string; author: User; score: number; note: string | null; tastedAt: string; createdAt: string; updatedAt: string; photoId: string | null; legacyPhotoMissing: boolean; comments: Comment[] }
...
export interface CreateRatingInput { photoIds?: string[]; groupId: string; itemId?: string; rereviewOf?: string; name?: string; brand?: string | null; variant?: string | null; type?: string | null; broadCategory?: string | null; score: number; note?: string | null; tastedAt?: string; photoId: string; idempotencyKey?: string }
export interface UpdateRatingInput { photoIds?: string[]; score?: number; note?: string | null; tastedAt?: string; photoId?: string }
